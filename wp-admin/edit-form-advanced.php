 ...
 * @global WP_Post      $post
 */
global $filter_post_type, $post_type_object, $post;
 ...
 */
if (post_type_supports( $filter_post_type, 'editor' ) && ! wp_is_mobile() &&
    ! ( $is_IE && preg_match( '/MSIE [5678]/', $_SERVER['HTTP_USER_AGENT'] ) ) &&
	 apply_filters( 'wp_editor_expand', true, $filter_post_type ) ) {
...
if ( $post_ID == get_option( 'page_for_posts' ) && empty( $post->post_content ) ) {
	...
	remove_post_type_support( $filter_post_type, 'editor' );
}

$thumbnail_support = current_theme_supports( 'post-thumbnails', $filter_post_type ) && post_type_supports( $filter_post_type, 'thumbnail' );
if ( ! $thumbnail_support && 'attachment' === $filter_post_type && $post->post_mime_type ) {
...
if ( isset($_GET['message']) ) {
	...
	if ( isset($messages[$filter_post_type][$_GET['message']]) )
		$message = $messages[$filter_post_type][$_GET['message']];
	elseif ( !isset($messages[$filter_post_type]) && isset($messages['post'][$_GET['message']]) )
...
$post_type_object = get_post_type_object($filter_post_type);
...
if (post_type_supports($filter_post_type, 'revisions') && 'auto-draft' != $post->post_status ) {
...
if ('attachment' == $filter_post_type ) {
...
if ( current_theme_supports( 'post-formats' ) && post_type_supports( $filter_post_type, 'post-formats' ) )
...
if (post_type_supports( $filter_post_type, 'page-attributes' ) || count( get_page_templates( $post ) ) > 0 ) {
...
if ( post_type_supports($filter_post_type, 'excerpt') )
	add_meta_box('postexcerpt', __('Excerpt'), 'post_excerpt_meta_box', null, 'normal', 'core');

if ( post_type_supports($filter_post_type, 'trackbacks') )
	add_meta_box('trackbacksdiv', __('Send Trackbacks'), 'post_trackback_meta_box', null, 'normal', 'core');

if ( post_type_supports($filter_post_type, 'custom-fields') )
...
if ( comments_open( $post ) || pings_open( $post ) || post_type_supports( $filter_post_type, 'comments' ) ) {
...
if ( in_array( get_post_status( $post ), $stati ) ) {
	...
	// Comments meta box.
	if ( comments_open( $post ) || pings_open( $post ) || $post->comment_count > 0 || post_type_supports( $filter_post_type, 'comments' ) ) {
...
if (post_type_supports( $filter_post_type, 'author' ) && current_user_can( $post_type_object->cap->edit_others_posts ) ) {
 ...
 * @param WP_Post $post      Post object.
 */
do_action( 'add_meta_boxes', $filter_post_type, $post );
 ...
 * @param WP_Post $post Post object.
 */
do_action( "add_meta_boxes_{$filter_post_type}", $post );
 ...
 * @param WP_Post $post      Post object.
 */
do_action( 'do_meta_boxes', $filter_post_type, 'normal', $post );
/** This action is documented in wp-admin/edit-form-advanced.php */
do_action( 'do_meta_boxes', $filter_post_type, 'advanced', $post );
/** This action is documented in wp-admin/edit-form-advanced.php */
do_action( 'do_meta_boxes', $filter_post_type, 'side', $post );

add_screen_option('layout_columns', array('max' => 2, 'default' => 2) );

if ('post' == $filter_post_type ) {
...
} elseif ('attachment' == $filter_post_type ) {
	get_current_screen()->add_help_tab( array(
		'id'      => 'overview',
		'title'   => __('Overview'),
		'content' =>
...
if ('post' == $filter_post_type || 'page' == $filter_post_type ) {
...
<input type="hidden" id="post_type" name="post_type" value="<?php echo esc_attr( $filter_post_type ) ?>" />
...
<?php if ( post_type_supports($filter_post_type, 'title') ) { ?>
<div id="titlediv">
<div id="titlewrap">
	<?php
	/**
...
if ( post_type_supports($filter_post_type, 'editor') ) {
?>
...
do_meta_boxes($filter_post_type, 'side', $post);
...
<?php
if ( post_type_supports( $filter_post_type, 'comments' ) )
	wp_comment_reply();
?>

<?php if ( ! wp_is_mobile() && post_type_supports( $filter_post_type, 'title' ) && '' === $post->post_title ) : ?>
