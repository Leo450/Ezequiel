 ...
 */
global $filter_post_type, $post_type_object;

$filter_post_type = $typenow;
...
if ('post' != $filter_post_type ) {
	$parent_file = "edit.php?post_type=$filter_post_type";
	$submenu_file = "edit.php?post_type=$filter_post_type";
	$post_new_file = "post-new.php?post_type=$filter_post_type";
} else {
...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			$post_ids = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type=%s AND post_status = %s", $filter_post_type, $post_status ) );
		}
		$doaction = 'delete';
...
add_screen_option( 'per_page', array( 'default' => 20, 'option' => 'edit_' . $filter_post_type . '_per_page' ) );
...
foreach ( $bulk_counts as $message => $count ) {
	if ( isset( $bulk_messages[ $filter_post_type ][ $message ] ) )
		$messages[] = sprintf( $bulk_messages[ $filter_post_type ][ $message ], number_format_i18n( $count ) );
	elseif ( isset( $bulk_messages['post'][ $message ] ) )
		$messages[] = sprintf( $bulk_messages['post'][ $message ], number_format_i18n( $count ) );

	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		$ids = preg_replace( '/[^0-9,]/', '', $_REQUEST['ids'] );
		$messages[] = '<a href="' . esc_url( wp_nonce_url( "edit.php?post_type=$filter_post_type&doaction=undo&action=untrash&ids=$ids", "bulk-posts" ) ) . '">' . __('Undo') . '</a>';
	}
}
...
<input type="hidden" name="post_type" class="post_type_page" value="<?php echo $filter_post_type; ?>" />
